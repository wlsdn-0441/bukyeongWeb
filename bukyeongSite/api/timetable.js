...
export default async function handler(req) {
  ...
  try {
    ...
    const grade = url.searchParams.get('grade') || "2"; // 기본값: 2학년
    const classNum = url.searchParams.get('classNum') || "6"; // 기본값: 6반
    ...
    const GRADE = grade;
    const CLASS_NM = classNum;

    const apiUrl = `https://open.neis.go.kr/hub/hisTimetable?KEY=${API_KEY}&Type=json&pIndex=1&pSize=100&ATPT_OFCDC_SC_CODE=${ATPT_OFCDC_SC_CODE}&SD_SCHUL_CODE=${SD_SCHUL_CODE}&ALL_TI_YMD=${ALL_TI_YMD}&GRADE=${GRADE}&CLASS_NM=${CLASS_NM}`;
